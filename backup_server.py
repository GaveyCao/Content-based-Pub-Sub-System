...
# send toggle back up message to virtual server
def toggle_backup():
    print("Central Server is down!\n")
    print("Back up is on!\n")
    channel = grpc.insecure_channel(virtualServer)
    stub = pr_pb2_grpc.PublishTopicStub(channel)
    response = stub.upgradeBackup(pr_pb2.empty())
    print(response.ack)
    return
...
'''
gRPC back up central server 
'''
class BackUpServer(pr_pb2_grpc.PublishTopicServicer):
    ...
    def commit_request(self, request, context):
        ...
        while (True):
            try:
                response = stub.commit_phase_two(request, context)
...
def serve():
    ...
    pr_pb2_grpc.add_PublishTopicServicer_to_server(BackUpServer(), server)
...
if __name__ == '__main__':
    ...
    # Hearing hear beat from central_server
    Heart = True
    while Heart:
        time.sleep(2)
        try:
            ...
            for item in dataRecieved["frontend"]:
                #print(item)
                if frontends.count_documents(item) > 0:
                    continue
                else:
                    frontends.insert_one(item)
            for item in dataRecieved["twoleveldict"]:
                #print(item)
                if twoLevelDict.count_documents(item) > 0:
                    continue
                else:
                    twoLevelDict.insert_one(item)
        except:
            print("Can't detect heart beat!\n")
            Heart = False
